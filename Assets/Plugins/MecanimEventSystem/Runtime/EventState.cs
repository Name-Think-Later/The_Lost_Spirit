...
namespace zFrame.Event
{
    ...
    public class EventState
    {
        ...
        public EventState(EventInfo eventInfo, int frame = -1) {
            ...
            EventHandler.GenerateAnimationEvent(a_Event, _keyFrame); //将事件绑定稍作提前，方便统一声明，避免调用过程中的 Rebind 
        }
        ...
        protected void ConfigEvent(int frame, Action<AnimationEvent> action) {
            ...
            if (!actions.Contains(action)) {
                ...
                if (EventHandler.LogLevels.HasFlag(LogLevel.Warning)) {
                    Debug.LogWarning(
                        $"AnimatorEventSystem[一般]：指定AnimationClip【{a_Event.animationClip.name}】已经订阅了该事件【{action.Method.Name}】！\n 建议：请勿频繁订阅！");
                }
            }
        }
