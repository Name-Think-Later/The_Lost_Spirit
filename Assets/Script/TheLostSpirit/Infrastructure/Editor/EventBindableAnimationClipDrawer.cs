    ...
    public class EventBindableAnimationClipDrawer : PropertyDrawer
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property) {
            ...
            if (iterator.NextVisible(true)) {
                do {
                    ...
                    if (iterator.name == nameof(EventBindableAnimationClip.animationClip)) {
        ...
        public class TimelineController
        {
            public TimelineController(
                ...
                _clipProperty = property.FindPropertyRelative(nameof(EventBindableAnimationClip.animationClip));
            }
            ...
            private void OnPlayModeStateChanged(PlayModeStateChange state) {
                ...
                // 直接停用整個 TrackView (包含所有 Marker)
                // 這會讓所有 Manipulator 自動失效
                _trackRoot?.SetEnabled(!isPlaying);

                // 如果你也想鎖住 Inspector 面板，也可以這樣做：
                // _inspectorContainer?.SetEnabled(!isPlaying);

                // 當我們要離開 Edit Mode (即將進入 Play Mode) 時
                // 必須強制停止 AnimationMode，釋放物件控制權給 Animator
            ...
            void AddEvent(float time) {
                ...
                var newEvent = new AnimationEvent
                    { time = time, functionName = FIXED_EVENT_NAME, stringParameter = "" };
                events.Add(newEvent);
            ...
            void OnEventFrameModified(int newFrame) {
                ...
                if (IsFrameOccupied(newFrame, _selectedEventIndex)) {
                    ...
                    // Revert visual
                    var currentFrame =
                        TimelineMath.TimeToFrame(events[_selectedEventIndex].time, _currentClip.frameRate);
            ...
            void UpdateDurationVisuals() {
                ...
                var action      = _currentEventData.combatActions[selectedIdx];
            ...
            void UpdateMarkerSelectionVisuals() {
                for (var i = 0; i < _markerLayer.childCount; i++) {
                    ...
                    marker.style.backgroundColor =
                        i == _selectedEventIndex ? Styles.MarkerSelected : Styles.MarkerNormal;
                }
            }
            ...
            void DrawEventInspector() {
                ...
                _inspectorFrameField =
                    new IntegerField { value = currentFrame, isDelayed = true, style = { width = 50 } };
