...
namespace TheLostSpirit.Infrastructure.Editor.EventBindableAnimationClip
{
    public class TimelineEditor : ITimelineController
    {
        ...
        void OnInspectorFrameChanged(int newFrame) {
            ...
            var targetTime = TimelineMath.FrameToTime(newFrame, _currentClip.frameRate);
            targetTime = TimelineMath.ClampTime(targetTime, _currentClip.length);
            ...
            if (events
                ...
                // Revert logic is handled by UI simply not updating? 
                // Or we should force update UI back.
                // Legacy calls `_inspectorFrameField.SetValueWithoutNotify`.
                // Here `OdinInspectorView` needs to handle revert or we just re-select current.
                SelectEvent(_selectedEventIndex); // Force refresh UI

                return;
            }

            MoveEvent(_selectedEventIndex, targetTime);
        }
    }
}